...
const sections = [
  {
    label: "01 — HYPER",
    title: "Lamborghini\nUrus SE",
    desc: "800 CV Hybrid. High-speed engineering for the Houston skyline.",
    tags: ["PHEV", "800 CV", "ELITE"],
    color: "#FFFFFF",
    accent: "#CBFF00"
  },
  {
    label: "02 — EXOTIC",
    title: "Lamborghini\nRevuelto",
    desc: "V12 Vision. A naturally aspirated hybrid masterpiece.",
    tags: ["V12 PHEV", "1015 CV", "AERO"],
    color: "#FFFFFF",
    accent: "#000000"
  },
  {
    label: "03 — LUXURY",
    title: "Mansory\nWraith",
    desc: "Bespoke Power. Forged carbon and unparallelled luxury.",
    tags: ["V12 BITURBO", "BESPOKE", "CARBON"],
    color: "#FFFFFF",
    accent: "#CBFF00"
  },
  {
    label: "04 — TRACK",
    title: "Corvette\nZ06",
    desc: "Flat-Plane Soul. track-focused performance for the Texas heat.",
    tags: ["V8", "670 HP", "Z07"],
    color: "#FFFFFF",
    accent: "#000000"
  }
    ...
    <div className="relative z-10 pointer-events-none" ref={containerRef}>
      ...
      {sections.map((section, i) => (
          ...
          >
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none select-none overflow-hidden w-full h-full flex items-center justify-center opacity-[0.05]">
              <span className="text-[60vh] font-display font-black tracking-tighter uppercase" style={{ color: section.accent === '#000000' ? '#000000' : '#000000' }}>
                {i + 1}
              </span>
            </div>

            <div className={`max-w-xl pointer-events-auto border-4 border-kinetic-black bg-white p-12 brutal-shadow ${i % 2 === 0 ? 'text-left' : 'text-right'}`}>
              <div 
                className="font-mono text-xs font-bold uppercase tracking-[0.2em] mb-6 inline-block px-2 py-1 bg-kinetic-black text-kinetic-white"
              >
                {section.label}
              </div>
              
              <h2 className="text-6xl md:text-8xl font-display mb-8 leading-[0.8] tracking-tight whitespace-pre-line uppercase text-kinetic-black">
                {section.title}
              </h2>

              <div className={`flex flex-col gap-8 ${i % 2 === 0 ? 'items-start' : 'items-end'}`}>
                <p className="text-kinetic-black/80 font-mono text-sm leading-relaxed max-w-sm uppercase">
                  {section.desc}
                </p>
                ...
                <div className="flex flex-wrap gap-2">
                  {section.tags.map((tag, j) => (
                    <span 
                      key={j} 
                      className="px-4 py-2 border-2 border-kinetic-black font-mono text-[10px] font-bold uppercase tracking-widest transition-all hover:bg-kinetic-accent"
                    >
                      {tag}
                    </span>
                  ))}

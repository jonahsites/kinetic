...
const Inventory: React.FC<InventoryProps> = ({ onClose }) => {
  ...
  return (
    <motion.div 
      ...
      className="fixed inset-0 z-[200] bg-kinetic-white overflow-y-auto px-6 py-10 md:px-16 border-l-4 border-kinetic-black"
    >
      <div className="max-w-[1400px] mx-auto">
        <div className="flex justify-between items-start mb-20 border-b-4 border-kinetic-black pb-12">
          <div>
            <div className="bg-kinetic-black text-kinetic-accent text-[10px] font-mono px-4 py-1 mb-4 w-fit font-bold uppercase tracking-widest">KINETIC DISPATCH — Houston</div>
            <h2 className="text-8xl lg:text-[10vw] leading-none">
              THE<br/><span className="text-outline-brutal">FLEET</span>
            </h2>
          </div>
          <button 
            onClick={onClose}
            className="p-6 border-4 border-kinetic-black bg-kinetic-white hover:bg-kinetic-accent brutal-shadow transition-colors"
          >
            <X size={32} />
          </button>
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-8 justify-between items-start mb-16">
          <div className="flex flex-wrap gap-2">
            {categories.map(cat => (
              <button
                ...
                className={`px-6 py-3 border-2 border-kinetic-black font-mono text-[10px] font-bold uppercase tracking-widest transition-all ${
                  activeCategory === cat ? 'bg-kinetic-black text-kinetic-white' : 'bg-transparent text-kinetic-black hover:bg-kinetic-accent'
                }`}
              >
                {cat}
              </button>
          ...
          <div className="relative w-full lg:w-96">
            <input 
              ...
              className="w-full bg-transparent border-b-4 border-kinetic-black py-4 px-2 font-mono text-xs uppercase tracking-widest focus:outline-none focus:bg-kinetic-accent transition-colors placeholder:text-kinetic-black/20"
            />
            <Search className="absolute right-4 top-1/2 -translate-y-1/2 text-kinetic-black" size={20} />
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-0 border-t-2 border-l-2 border-kinetic-black">
          <AnimatePresence mode="popLayout">
            {filteredCars.map((car) => (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="group relative border-r-2 border-b-2 border-kinetic-black p-8 bg-white hover:bg-kinetic-black transition-colors duration-500"
              >
                <div className="aspect-video overflow-hidden relative mb-8 border-2 border-kinetic-black">
                  <img 
                    src={car.image} 
                    alt={car.name} 
                  ...
                  />
                  <div className="absolute top-0 right-0 bg-kinetic-accent border-l-2 border-b-2 border-kinetic-black px-4 py-2">
                    <span className="font-mono text-xs font-black text-kinetic-black">${car.price}/D</span>
                  </div>
                </div>

                <div>
                  <div className="font-mono text-[10px] font-bold text-kinetic-black/40 group-hover:text-kinetic-accent transition-colors mb-2">{car.category}</div>
                  <h3 className="text-4xl font-display leading-[0.9] mb-8 group-hover:text-kinetic-white transition-colors">{car.name}</h3>
                  
                  <div className="grid grid-cols-2 gap-8 border-t-2 border-kinetic-black pt-6 group-hover:border-kinetic-white/20">
                    <div>
                      <div className="font-mono text-[9px] uppercase text-kinetic-black/40 group-hover:text-kinetic-white/40 mb-1">OUTPUT</div>
                      <div className="font-mono text-xs font-bold group-hover:text-kinetic-accent">{car.hp} HP</div>
                    </div>
                    <div>
                      <div className="font-mono text-[9px] uppercase text-kinetic-black/40 group-hover:text-kinetic-white/40 mb-1">TERMINAL</div>
                      <div className="font-mono text-xs font-bold group-hover:text-kinetic-accent">{car.speed}</div>
                    </div>
                  </div>
                </div>

                <button className="absolute bottom-4 right-4 w-12 h-12 bg-kinetic-accent border-2 border-kinetic-black flex items-center justify-center translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all">
                  <ArrowUpRight size={24} className="text-kinetic-black" />
                </button>
              </motion.div>
            ))}
          </AnimatePresence>
        ...
        {filteredCars.length === 0 && (
          <div className="py-40 text-center border-t-2 border-kinetic-black">
            <p className="font-mono text-xl uppercase tracking-widest">ZERO MATCHES IN SEGMENT.</p>
          </div>
        )}
      </div>
    </motion.div>
